...
SKIP = 1
MAXFRAME = 5000 # The number of simulation frames that will be analysed. The larger this is, the less noisy the correlation function.
TMAX = 750 # The number of frames up to which the correlation function will be analysed.
CUTOFF = 5
...
t0 = time()
fluc_corr_func, fluc_timescales = tools.summarize_correlation(fluc_data, ap_indices, dt, TMAX, CUTOFF, SKIP, MAXFRAME)
fixed_corr_func, fixed_timescales = tools.summarize_correlation(fixed_data, ap_indices, dt, TMAX, CUTOFF, SKIP, MAXFRAME)
print('Autocorrelation analysis took {0:.1f} minutes',(time() - t0) /60.0)
...
print('Effective interaction time for osmostat (ns) = ', np.mean(fluc_timescales), '+/', 2.0*np.std(fluc_timescales)/np.sqrt(len(fluc_timescales)))
print('Effective interaction time for fixed-fraction (ns) = ', np.mean(fixed_timescales), '+/', 2.0*np.std(fixed_timescales)/np.sqrt(len(fixed_timescales)))

# Bootstrap confidence estimates and confidence intervals
NBOOTS = 10000
fluc_corr_samps, fluc_time_samps = tools.bootstrap_correlation_functions(fluc_corr_func, dt, NBOOTS)
fixed_corr_samps, fixed_time_samps = tools.bootstrap_correlation_functions(fixed_corr_func, dt, NBOOTS)
print('Bootstrap fixed fraction timescale (ns) = ', np.mean(fluc_time_samps), '(',np.percentile(fluc_time_samps, 2.5), 'to', np.percentile(fluc_time_samps, 97.5),')' )
print('Bootstrap fixed fraction timescale (ns) = ', np.mean(fixed_time_samps), '(',np.percentile(fixed_time_samps, 2.5), 'to', np.percentile(fixed_time_samps, 97.5),')' )
...
print('Acceptance probability for saltswap =', acc_prob)
...
plt.savefig('dna_autocorrelation.png', dpi=DPI)
